...
const ParentChat = ({navigation}) => {
  ...
  const handleSendMessage = async(teacherId,teacherUsername,teacherImageURL) => {
    ...
    try {
      const TeacherChatSnapshot = await firestore().collection('TeachersChat').where('teacherId', '==',teacherId).get();
    
        const docId = TeacherChatSnapshot.docs[0].id;
    
        await firestore().collection('TeachersChat').doc(docId).update({
          unread: false,
      ...
      console.error("Error fetching TeachersChat:", error);
    }
