...
const SpecificPayments = ({ route, navigation }) => {
    ...
    const storeImage = async (selectedImage) => {
        try {
          ...
          fetchPayments();
          setLoading(false);
          Alert.alert('Success', 'Payment is Added Successfully');
      ...
      const fetchPayments = async () => {
        try {
            ...
            const querySnapshot = await paymentsRef
                .where('parentEmail', '==', data.email)
                .where('studentEmail', '==', data1.email)
...
export default SpecificPayments;
