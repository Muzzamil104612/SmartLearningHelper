...
import { parentemail } from '../../redux/action';
import { useDispatch } from 'react-redux';
import { useSelector } from 'react-redux';

const EditParentProfile = ({ navigation }) => {
  ...
  const fetchDataFromFirestore = async () => {
    try {
      const docRef = await firestore().collection('Parents').doc(myObject.userID).get();
  ...
  const [myObject1, setMyObject1] = useState({
    emailError: '', nameError: '', PhoneError: '', imageError: '',
  });
  ...
  const handleRegistration = async () => {
    ...
    if (myObject.name.trim().length < 3 || myObject.email.trim() === '' || myObject.ImageURL.trim() === '' ||
