...
import {  teacheremail} from '../../redux/action';
...
const handleRegistration = async () =>  {
    ...
    if (myObject.name.trim().length < 3 ||
     myObject.email.trim() === '' ||
          myObject.experience.trim() === '' ||
           myObject.majorSubject.trim() === '' ||
      myObject.qualification.trim() === '' ||
      myObject.ImageURL.trim() === ''||
      (!/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/.test(myObject.email)) 
      || myObject.password.trim() === ''||
