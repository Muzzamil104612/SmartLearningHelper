...
const SpecificTeacherPayment = ({ route, navigation }) => {
  ...
  const handleAcceptPayment = async (paymentId) => {
    try {
      setLoading(true);
      await firestore().collection('AdminPayment').doc(paymentId).update({
        Status: 'Accepted',
      });
      fetchPayments(); // Fetch payments again after updating the status
      setLoading(false);
      setShowButtons(false); // Hide buttons after accepting
      Alert.alert(Error, 'Payment is Accepted Successfully');
    } catch (error) {
...
export default SpecificTeacherPayment;
